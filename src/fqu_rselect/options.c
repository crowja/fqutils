...
options_new( void )
{
   ...
   tp->fname = NULL;
   tp->frac = 1.5;                               /* larger than unity, just to be safe */
   tp->max_threads = 1;
   tp->verbosity = 0;
...
options_helpmsg( FILE *out )
{
   ...
   fprintf( out, "Randomly select sequences from stdin, or <infile1> ... if specified, writing to stdout.\n" );
   fprintf( out, "\nOPTIONS\n" );
   fprintf( out, "%s\n", "-F, --fraction <frac>" );
   fprintf( out, "%s%s\n", indent, "Randomly select <frac> proportion of sequences from the input [default: 1]." );
   fprintf( out, "%s\n", "-h, --help" );
...
options_cmdline( struct options *p, int argc, char *argv[] )
{
   ...
   while ( 1 ) {
      ...
      c = getopt_long( argc, argv, "F:hqT:Vv", long_options, &option_index );
